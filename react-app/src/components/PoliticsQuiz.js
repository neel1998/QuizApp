...
var topics=new Set();
class PoliticsQuiz extends Component {
  constructor() {
    super();
     this.state = {
      data: [],
 ...
 componentDidMount() {
  ...
  fetch('http://localhost:8080/deletequiz/'+name+"/politics", {
        method: 'DELETE'
      }).then(response=>{
    ...
    return (
      ...
      <div className="App">
        <header className="App-header">
          <h1 className="App-title">Politics Quiz</h1>
        </header>
         {
          this.state.top.map(
            function(item,key){
       ...
       <h4 align="center">Previous Attempts</h4>
      {
        this.state.data.map(
            function(item,key){
              if(item.Username==localStorage.getItem("user") && item.Category==localStorage.getItem("Topic")){
